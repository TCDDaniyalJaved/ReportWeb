// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
using System;
using System.Collections.Generic;

namespace Accounting.Models;

public partial class JorderM
{
    public int Id { get; set; }

    public DateOnly? Date { get; set; }

    public string Remarks { get; set; }

    public int? PartyId { get; set; }

    public int RefNo { get; set; }

    public string DrefNo { get; set; }

    public int? CompanyId { get; set; }

    public DateTime? CurrentDate { get; set; }

    public string InputType { get; set; }

    public int? UserId { get; set; }

    public int? ItemId { get; set; }

    public int? QualityId { get; set; }

    public int? BrandId { get; set; }

    public string Border { get; set; }

    public decimal Dquantity { get; set; }

    public DateOnly? DueDate { get; set; }

    public virtual Company Company { get; set; }

    public virtual ICollection<JorderD2> JorderD2s { get; set; } = new List<JorderD2>();

    public virtual ICollection<JorderD3> JorderD3s { get; set; } = new List<JorderD3>();

    public virtual ICollection<JorderD> JorderDs { get; set; } = new List<JorderD>();

    public virtual Chart Party { get; set; }

    public virtual ICollection<ProcessM> ProcessMs { get; set; } = new List<ProcessM>();

    public virtual User User { get; set; }
}