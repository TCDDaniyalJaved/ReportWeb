DROP PROCEDURE IF EXISTS [PurchaseReport_Static]
GO
...
CREATE PROCEDURE [PurchaseReport_Static]
...
BEGIN
    ...
    SET @DateFrom = CONVERT(DateTime, @DateFrom1, 101);
    SET @DateTo   = CONVERT(DateTime, @DateTo1, 101);

    -- Temporary table with invoice details
    SELECT 
    ...
    -- Final select with static filters
    SELECT 
        A.SInputType, 
    ...
    ORDER BY 
        A.RefNo;  -- static ordering on RefNo
