...
public class CashPaymentController : Controller
{
    ...
    private static readonly string BasePath = "Apps/Accounting/CashPayment";
    ...
    public async Task<IActionResult> Create(CashPaymentViewModel model)
    {
        bool isAjax = Request.Headers["X-Requested-With"] == "XMLHttpRequest";
    ...
    {
        var codes = _context.CashPaymentMs
            .AsNoTracking()
    ...
    public async Task<IActionResult> Delete(int id)
    {
        try
        {
            var details = await _context.CashPaymentDs.Where(d => d.PersonId == id).ToListAsync();
            _context.CashPaymentDs.RemoveRange(details);

            var masterData = await _context.CashPaymentMs.FindAsync(id);

            if (masterData != null)
            {
                _context.CashPaymentMs.Remove(masterData);
                await _context.SaveChangesAsync();
    ...
    {
        var Accounting = _context.CashPaymentMviews.ToList();
    ...
    {
        var master = _context.CashPaymentMs.FirstOrDefault(m => m.Id == id);
        if (master == null)
            return NotFound();

        var details = _context.CashPaymentDs.Where(d => d.PersonId == id).ToList();
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(CashPaymentViewModel model)
    {
        if (!ModelState.IsValid)
        {
        ...
        try
        {
            ...
            if (deletedDetails.Any())
                _context.CashPaymentDs.RemoveRange(deletedDetails);
            ...
            foreach (var detail in model.Details)
            {
                ...
                if (detail.Id == 0)
                    _context.CashPaymentDs.Add(detail);
                else
                {
                    var existing = await _context.CashPaymentDs
    ...
    public async Task<IActionResult> GetDataDetail()
    {
        try
        {
            ...
            var columnName = Request.Form[$"columns[{sortColumnIndex}][name]"].FirstOrDefault() ?? "CashPaymentId";

            var query = _context.CashPaymentDviews.AsQueryable();
